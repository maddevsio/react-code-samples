...
test('resourceListReadRequestAsync', () => {
  const fakeDispatch = (action) => {
    const { defer } = action.meta
    defer.resolve(action)
  }

  return expect(actions.resourceListReadRequestAsync(fakeDispatch, 'resource', { _limit: 1 }, 'resourceAlias')).resolves.toEqual(
    expect.objectContaining({
      type: actions.RESOURCE_LIST_READ_REQUEST_ASYNC,
      payload: { params: { _limit: 1 } },
      meta: {
        resource: 'resource',
        resourceAlias: 'resourceAlias',
        defer: { resolve: expect.any(Function), reject: expect.any(Function) },
        thunk: 'resourceListRead',
      },
    })
  )
})

test('resourceCreateRequestAsync', () => {
  const fakeDispatch = (action) => {
    const { defer } = action.meta
    defer.resolve(action)
  }

  const data = { field1: 'field1', field2: 'field2' }

  return expect(actions.resourceCreateRequestAsync(fakeDispatch, 'resource', data)).resolves.toEqual(
    expect.objectContaining({
      type: actions.RESOURCE_CREATE_REQUEST_ASYNC,
      payload: { data },
      meta: {
        resource: 'resource',
        defer: { resolve: expect.any(Function), reject: expect.any(Function) },
        thunk: 'resourceCreate',
      },
    })
  )
})
